...
En este tutorial veremos cómo usar Docker para el desarrollo de aplicaciones sencillas, aprendiendo a crear servicios con _Docker Compose_ y conocer la técnica de desarrollo de aplicaciones multicontenedor con _Docker Compose_
...
* https://github.com/ualmtorres/calificaciones[Calificaciones]
====
...
* De forma predeterminada, si no se crea ninguna red de interconexión entre los contenedores, Compose crea una red _bridge_ para que los contenedores puedan comunicarse entre sí. Estos contenedores pueden accederse entre sí por el nombre del servicio.
...
1. En la carpeta `api` crear el archivo https://github.com/ualmtorres/calificaciones/blob/master/api/requirements.txt[`requirements.txt`] con las dependencias de la API.
1. En carpeta `api/routes` crear los archivos:
  * https://github.com/ualmtorres/calificaciones/blob/master/api/routes/test.py[`test.py`] con una ruta de prueba.
  * https://github.com/ualmtorres/calificaciones/blob/master/api/routes/calificaciones.py[`calificaciones.py`] con la ruta de la API para obtener las calificaciones. Este código incluye los datos en JSON y dos endpoints: uno para obtener todas las calificaciones y otro para obtener la calificación de un estudiante concreto por su id.
1. En la carpeta `api` crear el archivo https://github.com/ualmtorres/calificaciones/blob/master/api/main.py[`main.py`] con el código principal de la API que arranca el servidor y define las rutas.
1. En la carpeta `api` crear el https://github.com/ualmtorres/calificaciones/blob/master/api/Dockerfile[`Dockerfile`] con las instrucciones para crear la imagen de la API
...
1. En la carpeta de la aplicación (un nivel por encima de `api`) crear el archivo https://gist.github.com/ualmtorres/15f3ea9bc15bc8d12200c53ccfb8dd19[`docker-compose.yml`] para poder ejecutar la API. El archivo `docker-compose.yml` lo creamos un directorio por encima de `api` porque la especificación del volumen del código de la API en el archivo `docker-compose.yml` se hace un nivel por encima del directorio `api`
...
1. En la carpeta `frontend` crear el archivo https://github.com/ualmtorres/calificaciones/blob/master/frontend/requirements.txt[`requirements.txt`] con las dependencias del frontend.
1. Crear una carpeta `src`. Dentro de ella:
...
1. En la carpeta `src/templates` crear el archivo https://github.com/ualmtorres/calificaciones/blob/master/frontend/src/templates/index.html[`index.html`] con la plantilla HTML para mostrar las calificaciones.
1. En la carpeta `src` crear el archivo https://github.com/ualmtorres/calificaciones/blob/master/frontend/src/app.py[`app.py`] con el código principal del front que arranca el servidor Flask y define la ruta principal que consume la API y renderiza la plantilla HTML.
1. Crear el https://github.com/ualmtorres/calificaciones/blob/master/frontend/Dockerfile[`Dockerfile`] con las instrucciones para crear la imagen del frontend
...
### Despliegue de aplicaciones con varios archivos Docker Compose
...
### Software adicional para trabajar con Docker

* https://www.portainer.io/[Portainer]: Interfaz web para gestionar contenedores Docker
* https://github.com/selfhosters-cc/container-census[Container Census]: Herramienta de gestión de contenedores Docker. Permite tener una visión global de los contenedores desplegados en distintos hosts Docker y funciones interesantes de análisis de vulnerabilidades, actualizaciones, etc.
